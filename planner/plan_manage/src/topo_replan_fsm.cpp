...
namespace fast_planner
{
  ...
  void TopoReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
  {
    ...
    if (false)
    {
      ...
      double dist = planner_manager_->pp_.dynamic_ ? edt_env->evaluateCoarseEDT(target_point_, /* time to program start */ info->duration_) : edt_env->evaluateCoarseEDT(target_point_, -1.0);

...
namespace fast_planner
{
                                             ...
                                             Eigen::Vector3d end_vel)
  {
    ...
    if ((start_pt - end_pt).norm() < 0.2)
    {
      ...
      ts = std::max(1.0, 0.5 * (start_pt - end_pt).norm() / pp_.max_vel_);
      point_set.push_back(start_pt);
      point_set.push_back(0.5 * (start_pt+end_pt));
      ...
      start_end_derivatives.push_back(Eigen::Vector3d(0.0, 0.0, 0.0));
    }else{
    ...
    while (!feasible && ros::ok())
    {
      ...
      cout << "[Main]: pos.getTimeSum() " << pos.getTimeSum() << endl;
      if (++iter_num >= 10)
        break;
    }

    if (!pos.checkFeasibility(true))
    {
      return false;
    }
  ...
  //   NonUniformBspline::parameterizeToBspline(1.0, point_set, start_end_derivatives, ctrl_pts);

  //   local_data_.position_traj_ = NonUniformBspline(ctrl_pts, 3, 1.0);
  ...
  void FastPlannerManager::planYaw(const Eigen::Vector3d &start_yaw)
  {
    ...
    double dt_yaw = 0.5;
    int seg_num = std::max(2, int(ceil(duration / dt_yaw)));
    dt_yaw = duration / seg_num;


    // auto &pos = local_data_.position_traj_;
    // double duration = pos.getTimeSum();
    // int seg_num = pos.getControlPoint().rows() - 3;

    // double dt_yaw = duration / seg_num; // time of B-spline segment
    // std::cout << "dt_yaw: " << dt_yaw << ", start yaw: " << start_yaw.transpose() << std::endl;
    ...
    for (int i = 0; i < seg_num; ++i) {
      ...
      //std::cout << "waypt is " << waypt.transpose() << std::endl;
    ...
    // std::cout << "[localPlanner]:  start_yaw " <<  start_yaw << std::endl;
    // waypts.push_back(start_yaw);
    // waypt_idx.push_back(0);
    ...
    //Eigen::Vector3d end_yaw = last_yaw3d;



    yaw.block(seg_num, 0, 3, 1) = states2pts * end_yaw;
    // std::cout << "yaw sequence is " << yaw  << std::endl;
    //  solve
    bspline_optimizers_[1]->setWaypoints(waypts, waypt_idx);
    int cost_func = BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::WAYPOINTS | BsplineOptimizer::FEASIBILITY;
    yaw = bspline_optimizers_[1]->BsplineOptimizeTraj(yaw, dt_yaw, cost_func, 1, 1);

    // std::cout << "start_yaw is " << start_yaw << std::endl;

    // std::cout << "dt_yaw is " << dt_yaw << std::endl;
                                   ...
                                   const double &end_yaw)
  {
    ...
    double dt_yaw = duration / seg_num; // time of B-spline segment
    std::cout << "dt_yaw: " << dt_yaw << ", start yaw: " << start_yaw.transpose()
    ...
    double init_dyaw = (new_end_yaw - start_yaw(0)) / duration;
    std::cout << "[localPlanner]: new_end_yaw" << new_end_yaw << std::endl;
    ...
    for (int i = 0; i < seg_num; ++i)
    {
      waypt(0) = start_yaw(0) + init_dyaw * i * dt_yaw;
      waypt(1) = waypt(2) = 0.0;
      waypts.push_back(waypt);
      waypt_idx.push_back(i);
    }
    ...
    Eigen::Vector3d end_yaw_3d(new_end_yaw, 0, 0);
    yaw.block(seg_num, 0, 3, 1) = states2pts * end_yaw_3d;

    // solve
    bspline_optimizers_[1]->setWaypoints(waypts, waypt_idx);
    int cost_func = BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::WAYPOINTS | BsplineOptimizer::FEASIBILITY | BsplineOptimizer::ENDPOINT;
    yaw = bspline_optimizers_[1]->BsplineOptimizeTraj(yaw, dt_yaw, cost_func, 1, 1);
  ...
  void FastPlannerManager::refineEndYaw(double &start_yaw, double &end_yaw)
  ...
  void FastPlannerManager::calcNextYaw(Eigen::Vector3d &next_yaw,
                                       Eigen::Vector3d &last_yaw,
                                       Eigen::Vector3d &dir,
                                       double dt)
  {

    double yaw_temp = dir.norm() > 1e-4
                          ? atan2(dir(1), dir(0))
                          : last_yaw(0);
    ...
    if (diff >= 2 * M_PI - diff)
    {
      ...
      else if (yaw_temp >= 0 && last_yaw(0) <= 0)
      {
        yaw_temp -= 2 * M_PI;
      }
    }
    ...
    // std::cout << " dd_yaw  " << d_yaw  << std::endl;
    ...
    next_yaw(1) =0.0;
    next_yaw(2) = 0.0;
    next_yaw(0) = last_yaw(0) + d_yaw * dt;

  // std::cout << "next_yaw is  " << next_yaw << std::endl;
